...
$(document).ready(function () {
  ...
  function drawVisualization() {
    ...
    $("#json-sim-chart-container").append(
      `<p id=${jsonSim.highID.slice(1)}>Highest recorded JSON Simulation value is ${jsonSim.highest}</p>`,
    );
    $("#json-sim-chart-container").append(
      `<p id=${jsonSim.lowID.slice(1)}>Lowest recorded JSON Simulation value is ${
        jsonSim.lowest
      }</p>`,
    );
    $("#ws-sim-chart-container").append(
      `<p id=${wsSim.highID.slice(1)}>Highest recorded WS Simulation value is ${wsSim.highest}</p>`,
    );
    $("#ws-sim-chart-container").append(
    ...
    $("#ajax-temp-chart-container").append(
      `<p id=${ajaxTemp.highID.slice(1)}>Highest recorded Ajax Temperature value is ${ajaxTemp.highest}</p>`,
    );
    $("#ajax-temp-chart-container").append(
      `<p id=${ajaxTemp.lowID.slice(1)}>Lowest recorded Ajax Temperature value is ${
        ajaxTemp.lowest
      }</p>`,
    );
    $("#ajax-air-chart-container").append(
      `<p id=${ajaxAir.highID.slice(1)}>Highest recorded Ajax Air value is ${ajaxAir.highest}</p>`,
    );
    $("#ajax-air-chart-container").append(
