(function (window, opspark, _) {
  ...
  _.set(opspark, "playa.space", function (messenger, level) {
    // holds all bodies active in our space //
    const // hitTest = phyz[level.hitTest]
    ...
    return {
      ...
      update(event) {
        var shipWithFreeze = active.find(function (body) {
          return body.FreezePower === true;
        });
        // console.log({shipWithFreeze})
        if (shipWithFreeze) {
          // move the ship
          shipWithFreeze.update(event);
          phyz.updatePosition(shipWithFreeze);
          // find that ships projectiles
          var unFroze = active.filter(function (body) {
            return body.type === "projectile" && body.emitter === shipWithFreeze;
          });
          unFroze.forEach((body) => {
  ...
  function getDistanceAttributes(bodyA, bodyB) {
    const distanceX = bodyB.x - bodyA.x,
  ...
  function handleCollision(distanceAttributes, hitResult, impactProperties) {
    const bodyA = distanceAttributes.bodyA,
    ...
    const springToX = bodyA.x + (distanceX / distance) * minimumDistance,
